...
var Game = {
	...
	createFood: function() {
		...
		_this.foodTimer = setInterval(function() {
			...
			if(_this.food.length == 0) { //如果地图上还没有食物
				...
				for(var i = 0; i < _this.snake.length; i++) {
					for(var j = 0; j < _this.temp.length; j++) {
						if(_this.snake[i].x == _this.temp[j].x && _this.snake[i].y == _this.temp[j].y) {
							_this.temp.splice(_this.temp[j], 1);
						}
					}
				}
				//从剩下不重复的位置随机出一个生成食物
				...
				_this.oF = {
					x: _this.temp[randomIndex].x,
					y: _this.temp[randomIndex].y
				}; //随机出地图上一个位置
				_this.ofb = true;
				if(_this.ofb == true) {
	...
	foodStyle: [{
		color: '#e40f29',
		shadow: '0 0 1px 1px #e40f29',
		score: 100
	}, {
		color: '#32df4b',
		shadow: '0 0 1px 1px #32df4b',
		score: 80
	}, {
		color: '#ec32f0',
		shadow: '0 0 1px 1px #ec32f0',
		score: 50
	}, {
		color: '#3598fa',
		shadow: '0 0 1px 1px #3598fa',
		score: 20
	}],
	//存放贪吃蛇的数组集合
	snake: [{
